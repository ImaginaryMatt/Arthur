...
const int stepsPerCommand = 6; //Number of steps per command sent. Needs to be set based on refresh rate
...
const float rotationTime = 0.40; //Half a second
...
void processCommand(String command) {
  ...
  case 1: // Left Motor
    if (killSwitch == false)
    {
      ...
      if (value >= 260)
      {
        pwmValue = map(value, 260, 510, 127, speedLimit);
      ...
      {
        pwmValue = map(value, 250, 0, 127, speedLimit);
...
void loop() {
  ...
  if (client)
  {
    ...
    processCommand(killItKillIt); //sending kill switch command either for shutdown or in the event of a disconnect
    client.stop();
