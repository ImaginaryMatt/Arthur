...
def send_command(sock, motor_id, value):
    ...
    if motor_id == 1 or motor_id == 2:
        output_values[motor_id-1] = int(value)
    else:
...
def extend_halt():
    send_command(sock,6,0) # Send the halt command for the extension motor to the robot
    print("Extend timer has finished")
    global extend_timer_running 
    extend_timer_running = False # Mark that the extension motor is not running

def grasp_halt():
    send_command(sock,7,0) # Send the halt command for the claw motor to the robot
    print("Grasp timer has finished")
    global grasp_timer_running
    grasp_timer_running = False # Mark that the claw motor is not running

def estop_reset():
    send_command(sock,8,2) # Send the re-enable command to the robot
    print("Estop Reset has finished")
    global killed
    killed = False # Mark the robot as live

def enable_on_press():
    global button_press_time
    if button_press_time is None: # If there is no value stored in button_press_time
        button_press_time = time.time() # Mark the time the button has been pressed
        print ("Beginning Re-Enable Attempt")

def enable_on_release():
    global button_press_time
    if button_press_time is not None: # If there is a value stored in button_press_time
        hold_duration = time.time() - button_press_time # Calculate how long the button has been held for
        print(hold_duration)
        if hold_duration >= 3: # If the button has been pressed for more than 3 seconds
            estop_reset() # Run the estop reset
            button_press_time = None # Clear the value of button_press_time
        else:
            button_press_time = None # Clear the value of button_press_time
    ...
    try:
        while True:
            ...
            if joystick.get_button(2):  # If button 2 is pressed
                ...
                send_command(sock, 3, 2)  # Run motor 3  CW (head right)
            elif joystick.get_button(3):  # If button 3 is pressed
                send_command(sock, 3, 3)  # Run motor to home position
            else:
                send_command(sock,3,0)    # Stop head motor
            ...
            if joystick.get_button(11) and extend_timer_running == False:  # If button 11 is pressed and the extension motor is not running
                send_command(sock, 6, 1)  # Extend arm
                extend_timer_running = True # Mark global variable for extension timer to true
                extend_timer.start_timer(extend_halt) # Start a timer that runs the halt command when it expires
            ...
            if joystick.get_button(14) and grasp_timer_running == False:  # If button 0 is pressed
                ...
                grasp_timer_running = True # Mark global variable for claw timer to true
                grasp_timer.start_timer(grasp_halt) # Start a timer that runs the halt command when it expires
            ...
            if joystick.get_button(4) and killed == False: # If start has been pressed and the robot has not yet been killed
            ...
            elif joystick.get_button(6) and estop_timer_running == False: # If the option button has been pressed and an estop timer is not running
                estop_timer_running = True # Mark that the estop timer is running
                enable_on_press() # Runs the initial re-enable function
            #Evaluate Re-Enable attempt when button 6 is released
            elif joystick.get_button(6) == False and estop_timer_running == True: # If the option button is released and an estop timer is running
                estop_timer_running = False # Mark that the estop timer is not running
                enable_on_release() # Runs the second re-enable function
            else:
                send_command(sock,8,0) #This does nothing but provide an output for our GUI
